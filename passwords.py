...
class PasswordListResource(Resource):
    ...
    def get(self):
        return g.user.passwords.query.all()
    ...
    def post(self):
        ...
        if not args['site'] or not args['email'] or not args['password']:
            abort(406)
        # Add password
        password = Password(g.user.id, args['site'], args['email'], args['password'])
